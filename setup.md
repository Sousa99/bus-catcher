...
- **`backend`** (`@sousa99/bus-catcher-backend`) — a single dual-mode package with two executions:
  - REST API via `--http`
  - MCP server via `--mcp`
...
- **`frontend`** (`@sousa99/bus-catcher-components`) — a single package producing three artifacts from
  the same source:
