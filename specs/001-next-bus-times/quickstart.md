...
- **Expected**: `status` returns a timestamp; `times` lists upcoming buses
  with `scheduledAt` (UTC) + `minutesUntil`; config round-trips.

**Validate MCP** (`contracts/mcp-tools.md`): connect an MCP client to
`http://localhost:3001/mcp` and invoke `get_stop_times`, `add_stop`,
`get_config`; confirm outputs match the REST responses for the same inputs.
